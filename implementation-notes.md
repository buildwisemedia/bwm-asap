...
- Added trusted-only `form_start` and `phone_click`, accepted-response `generate_lead`, strict source-ID deduplication, privacy-safe common fields, and a single direct GA4 producer. Existing Meta Lead dedup remains active for real accepted leads; synthetic canaries never emit Meta Lead.
- Replaced public query-string canary authority with a short-lived server-signed bridge placed by the operator in `sessionStorage`. The token is validated by POST with `no-referrer`, immediately removed from storage, never added to the URL, hidden inputs, FormData, dataLayer, GA4, Meta, or receipts, and cleared from memory after one accepted synthetic submit. The capture handler blocks Webflow and every legacy business submission path for valid, invalid, expired, or consumed canary intent and posts only the token plus fixed routing fields.
