...
async def inspect(browser,slug,route,hero_name,private,width,height,dpr):
    ...
    await page.locator('a[href="#estimate"]').first.click(); await settled(page,"#estimate"); anchor=await page.locator("#estimate").evaluate("n=>n.getBoundingClientRect().top")
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        output={"ok":True,"routes":len(ROUTES),"renders":len(results),"matrix":results,"noJs":await no_js(browser),"screenshots":str(OUT)};await browser.close()
